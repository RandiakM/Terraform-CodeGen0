...
const RouteTableForm = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Generate Route Table</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {routes.map((route, index) => (
          <div key={index} className="space-y-2">
            <h3 className="text-lg font-medium text-gray-700">Route {index + 1}</h3>
            <div>
              <label htmlFor={`cidrBlock-${index}`} className="block text-sm font-medium text-gray-700 mb-1">
                CIDR Block
              </label>
              <input
                type="text"
                ...
                onChange={(e) => handleRouteChange(index, 'cidrBlock', e.target.value)}
                required
                className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            <div>
              <label htmlFor={`gatewayId-${index}`} className="block text-sm font-medium text-gray-700 mb-1">
                Gateway ID
              </label>
              <input
                type="text"
                ...
                onChange={(e) => handleRouteChange(index, 'gatewayId', e.target.value)}
                required
                className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
